  ...
  return (
    <div className="app-shell" style={{ paddingBottom: subscribed ? 0 : undefined }}>
      <div className="subpage-header">
        <button className="back-btn" onClick={() => router.push(`/profile/${id}`)}>
          ←
        </button>
        <span className="title">
          {subscribed ? `${c.name}さんとのチャット` : "VIPプラン"}
        </span>
      </div>
      ...
      {!subscribed ? (
        <div className="page-content">
          <div style={{ textAlign: "center", padding: "4px 0 18px" }}>
            ...
            <p className="text-sub" style={{ fontSize: "11.5px", margin: 0 }}>
              月額プランに加入すると直接メッセージを送れます
            </p>
          </div>
          ...
          >
            <div style={{ display: "flex", alignItems: "baseline", gap: 4, marginBottom: 12 }}>
              <span style={{ fontSize: 24, fontWeight: 700, color: "var(--purple-dark)" }}>
                ¥500
              </span>
              <span style={{ fontSize: "11px", color: "var(--purple)" }}>/ 月</span>
            </div>
            {["夢追い人と1対1でチャット", "VIPバッジがプロフィールに表示", "いつでも解約可能"].map(
              (f) => (
                <div
                  key={f}
                  style={{
          ...
          <button onClick={handleSubscribe} className="btn btn-purple btn-block">
            👑 月額500円でVIPになる
          </button>
        </div>
      ) : (
              ...
              >
                ...
                <div
                  style={{
                    ...
                    fontSize: 13,
                    lineHeight: 1.65,
                    background: m.from === "user" ? "var(--coral-light)" : "var(--paper)",
                    color: m.from === "user" ? "var(--coral-dark)" : "var(--text-main)",
                    border: m.from === "creator" ? "1px solid var(--border)" : "none",
                  }}
                >
                  {m.text}
                </div>
