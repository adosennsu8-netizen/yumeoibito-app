    ...
    <div className="app-shell">
      ...
      {step === "verify" && (
        <div className="page-content">
          <p style={{ fontSize: "13.5px", fontWeight: 700, margin: "10px 0 8px" }}>
            本人確認書類のアップロード
          </p>
          ...
          <div
            ...
            style={{
              border: verifyFile ? "1.5px solid var(--coral)" : "1.5px dashed var(--text-faint)",
              borderRadius: "var(--radius-md)",
              padding: 30,
              textAlign: "center",
              marginBottom: 18,
              color: verifyFile ? "var(--coral-dark)" : "var(--text-faint)",
              background: verifyFile ? "var(--coral-light)" : "transparent",
              cursor: "pointer",
            }}
          >
            <span style={{ fontSize: 24, display: "block", margin: "0 auto 8px" }}>
              {verifyFile ? "✅" : "🪪"}
            </span>
          ...
          >
            <span>🔒</span>
            <span>
              アップロードされた書類は本人確認のみに利用され、運営は口座情報・書類画像を直接保持しません（決済代行・本人確認事業者が管理します）。審査には1〜2営業日ほどお時間をいただく場合があります。
            </span>
          </div>
          ...
          <button
            ...
            disabled={!verifyFile}
            className={verifyFile ? "btn btn-coral btn-block" : "btn btn-disabled btn-block"}
          >
      ...
      {step === "profile" && (
        <div className="page-content">
          ...
          <div className="field">
            <label>年齢</label>
            <input type="number" placeholder="例：28" min={18} max={99} style={{ maxWidth: 120 }} />
          </div>
          <div className="field">
          ...
          <div className="field">
            ...
            <input type="text" placeholder="例：写真家を目指して 2年目" />
          </div>
          <div className="field">
            <label>夢への一言</label>
            <textarea rows={3} className="serif-input" placeholder="あなたの夢を一言で表すと？" />
          </div>
          <div className="field">
            <label>自己紹介</label>
            <textarea rows={4} placeholder="どんな夢を、どんな想いで追いかけているか教えてください" />
          </div>
          ...
          >
            <span>ℹ️</span>
            <span>
              プロフィール写真は登録完了後にマイページから設定できます。過度な加工が施された写真は利用できません。
            </span>
          </div>

          <button onClick={handleProfileSubmit} className="btn btn-coral btn-block">
            登録を完了する
          </button>
        </div>
      )}
