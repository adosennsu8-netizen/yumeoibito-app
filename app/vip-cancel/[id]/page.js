...
export default function VipCancelPage() {
  ...
  const id = params.id;
  const c = CREATORS[id];
  const vipInfo = CURRENT_USER.vip.find((v) => v.creatorId === id) || {
    nextBilling: "次回更新日未定",
    price: 500,
  };
  ...
  return (
    <div className="app-shell">
      <div className="subpage-header">
        <Link className="back-btn" href="/mypage">
          ←
        </Link>
      ...
      {!done ? (
        <div className="page-content" style={{ paddingTop: 6 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 22 }}>
            ...
            />
            <div>
              <p style={{ fontSize: "14.5px", fontWeight: 700, margin: 0 }}>
                {c.name}さんのVIPプラン
              </p>
          ...
          >
            ...
            解約後も<strong>{vipInfo.nextBilling}まではVIPチャット・限定投稿を利用できます</strong>。
            期限を過ぎると自動的にVIP特典が終了します。
          </div>

          <p style={{ fontSize: "12.5px", color: "var(--text-faint)", margin: "0 0 10px", fontWeight: 700 }}>
            解約の理由（任意）
          </p>
          {reasons.map((r) => (
            <label
        ...
        <div className="page-content text-center" style={{ paddingTop: 56 }}>
          ...
          <Link className="btn btn-ghost btn-block" href="/mypage">
            マイページに戻る
          </Link>
        </div>
      )}
